...
def create_logger() :
    ...
    fh = logging.FileHandler('{}logs_output.log'.format(path_experiments))
...
def display_city(mtx,win_name=None,viz=None,folder_name=None,epoch=None) :
    """
    ...
    if viz: viz.histogram(mtd.flatten(), win=win_name + ' Histogram mtd', opts=dict(title='Histogram mtd'))
    showMapVisdom(mtd, viz,folder_name, generated=True, win_name=win_name, epoch=epoch)

def load_dataset(viz,folder_name) :
    ...
    if viz :
        ...
        display_city(real_batch[0][0], win_name='Training Example',viz=viz)
